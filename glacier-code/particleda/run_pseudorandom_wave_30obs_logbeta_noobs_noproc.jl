# Particle-tracking driver for the pseudo-random wave setup with 30 sensors,
# no observation assimilation, and no process noise.
#
# This mirrors the run14 pseudo-random/log-beta setup but turns off both the
# observation update and the stochastic model perturbation so we get a clean
# deterministic forecast baseline under the same truth and station layout.
#
# Run:
#   julia --project=test glacier-code/particleda/run_pseudorandom_wave_30obs_logbeta_noobs_noproc.jl
...
const OUT = joinpath("glacier-code", "particleda", "results",
                     "run15_pseudorandom_wave_30obs_logbeta_noobs_noproc")
mkpath(OUT)
const _SSD_BASE = "/Volumes/ZX20/USRA 2026"
const EXT_OUT = let candidate = joinpath(_SSD_BASE, "run15_pseudorandom_wave_30obs_logbeta_noobs_noproc")
    try
        mkpath(candidate)
        candidate
    catch
...
println("Setup (PSEUDORANDOM WAVE, 30 SENSORS, NO OBS, NO PROC NOISE): nprt=$NPRT, T=$T, n_state=$n_state, n_obs=$n_obs")
...
h5open(out_path, "w") do f
    ...
    attributes(g_par)["obs_space"] = model.parameters.obs_space
    attributes(g_par)["obs_noise_std"] = model.parameters.obs_noise_std
    attributes(g_par)["obs_noise_std_log"] = model.parameters.obs_noise_std_log
    attributes(g_par)["disable_observations"] = model.parameters.disable_observations
    attributes(g_par)["process_std_beta"] = model.parameters.process_std_beta
end

println("Saved tracking file to ", out_path)
