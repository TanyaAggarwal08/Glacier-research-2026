...
# using log(β) observations.
#
# This keeps the same truth, dynamics, and particle prior as run12, but the
# observation operator is H(β) = log(β) at the sensor cells. The observation
# noise is specified in log units so it represents a roughly constant
# fractional-error model across the full β range.
#
# Run:
#   julia --project=test glacier-code/particleda/run_pseudorandom_wave_30obs_logbeta.jl
...
const OUT = joinpath("glacier-code", "particleda", "results",
                     "run14_pseudorandom_wave_30obs_logbeta")
mkpath(OUT)
const _SSD_BASE = "/Volumes/ZX20/USRA 2026"
const EXT_OUT = let candidate = joinpath(_SSD_BASE, "run14_pseudorandom_wave_30obs_logbeta")
    try
        mkpath(candidate)
        candidate
    catch
...
println("Setup (PSEUDORANDOM WAVE, 30 OBS, LOG-BETA OBS): nprt=$NPRT, T=$T, n_state=$n_state, n_obs=$n_obs")
println("  obs_space=$(model.parameters.obs_space), obs_noise_std_log=$(model.parameters.obs_noise_std_log)")
