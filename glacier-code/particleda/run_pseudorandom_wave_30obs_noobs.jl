...
# present but no observation assimilation.
#
# This mirrors run12 exactly, except `disable_observations = true` so the
# particle filter performs a free forecast while keeping the same station
# geometry and saved observation stream for plotting/comparison.
#
# Run:
#   julia --project=test glacier-code/particleda/run_pseudorandom_wave_30obs_noobs.jl
...
const OUT = joinpath("glacier-code", "particleda", "results",
                     "run13_pseudorandom_wave_30obs_noobs")
mkpath(OUT)
const _SSD_BASE = "/Volumes/ZX20/USRA 2026"
const EXT_OUT = let candidate = joinpath(_SSD_BASE, "run13_pseudorandom_wave_30obs_noobs")
    try
        mkpath(candidate)
        candidate
    catch
...
println("Setup (PSEUDORANDOM WAVE, 30 SENSORS, NO ASSIMILATION): nprt=$NPRT, T=$T, n_state=$n_state, n_obs=$n_obs")
