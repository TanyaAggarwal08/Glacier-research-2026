...
#
# Same pseudo-random truth/background construction as run11, but with a denser
# observation network so we can compare RMSE directly against the 10-sensor run.
#
# Run: julia --project=test glacier-code/particleda/run_pseudorandom_wave_30obs.jl
...
const OUT = joinpath("glacier-code", "particleda", "results",
                     "run12_pseudorandom_wave_30obs")
mkpath(OUT)
const _SSD_BASE = "/Volumes/ZX20/USRA 2026"
const EXT_OUT = let candidate = joinpath(_SSD_BASE, "run12_pseudorandom_wave_30obs")
    try
        mkpath(candidate)
        candidate
    catch
...
println("Setup (PSEUDORANDOM WAVE, 30 OBS): nprt=$NPRT, T=$T, n_state=$n_state, n_obs=$n_obs")
