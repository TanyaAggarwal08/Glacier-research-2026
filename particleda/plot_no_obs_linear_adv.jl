# Visualisations for the LINEAR-advection NO-OBS run.
#
# Mirrors plot_linear_advection.jl but reads from run09_no_obs_linear_adv.
...
const TRACK = "/Volumes/ZX20/USRA 2026/run09_no_obs_linear_adv/tracking.h5"
...
anim = @animate for t in 1:T_plus_1
    p = plot(xs_km, data.truth[row_mid, :, t];
             ...
             title=@sprintf("LINEAR adv, NO OBS  y=%.0f km  t=%.1f h",
...
anim_all = @animate for t in 1:T_plus_1
    p = plot(; xlabel="x (km)", ylabel="β (Pa·s/m)",
             title=@sprintf("LINEAR adv, NO OBS — ALL %d particles  y=%.0f km  t=%.1f h",
...
anim_trail = @animate for T_now in 1:T_plus_1
    ...
    p = plot(; xlabel="time (h)", ylabel="β (Pa·s/m)",
             title=@sprintf("LINEAR adv, NO OBS — β(t) at sensor (i=%d,j=%d)  t=%.1f h",
...
p_ess = plot(ww, data.ess; xlabel="time (h)", ylabel="ESS",
             title="LINEAR adv, NO OBS — Effective Sample Size",
