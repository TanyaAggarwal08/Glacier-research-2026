...
# Centre of the β field. Lowering it makes the bed slipperier overall, which
# matters a lot under linear sliding (m=1): τ_b = β·u, so speed ≈ τ_d/β and a
# centre of 2000 pins the interior to only tens of m/yr. 1000+1000·sin·sin
# (centre 1000, amplitude 1000) reproduces the original main.jl field.
const PRIOR_CENTER = parse(Float64, _arg(13, "2000.0"))
...
p = IceExpDyn.Params(
    ...
    prior_center_beta=PRIOR_CENTER, prior_signal_scale_beta=300.0,
...
println("prior_mode=$PRIOR_MODE center=$PRIOR_CENTER amplitude=$PRIOR_AMP n_modes=$PRIOR_NMODES")
