# Experiment 11 — RMSE: n_obs sweep with σ_proc = 0 AND Lax–Wendroff advection.
#
# Purpose: isolate whether the RMSE decay we saw on the n_obs=0 line in
# Exp 10 was real filter learning or an artifact of the first-order
# upwind stencil's numerical diffusion. Lax–Wendroff has D_num ≡ 0 to
# second order (see glacier-notes/21_numerical_diffusion.md), so if the
# diffusion diagnosis is right, the n_obs=0 curve here should stay flat.
#
# Everything else matches Exp 10: same σ_init, σ_obs, ℓ, seeds, grid,
# sensor pool. Only advection_type flips from "linear" → "lax_wendroff".
#
# Also switches ε=0 so LW is strictly linear (no β-feedback in v).
#
# Runs under --project=test.
# Run: julia --project=test glacier-code/particleda/rmse_experiments/experiment_11_n_obs_sweep_lw_no_procnoise.jl
...
const FIG_NAME = "rmse_n_obs_sweep_lw_no_procnoise_exp11.png"
const NOTES    = "Lax–Wendroff advection; σ_proc = 0; n_obs sweep. n=0 curve should be truly flat."

base_params = Dict{String, Any}(
    "nx" => 40, "ny" => 40,
    "x_length" => 160_000.0, "y_length" => 160_000.0,
    "init_std_beta"      => 600.0,
    "process_std_beta"   => 0.0,
    "obs_noise_std"      => 0.10,
...
# Sensor pool — same seed as Exp 07 / 10 for direct comparability.
const STATION_DIR = joinpath(@__DIR__, "_tmp_stations_exp11")
mkpath(STATION_DIR)
...
function _write_station_file(n::Int)
    ...
    open(path, "w") do io
        println(io, "# Auto-generated for experiment_11 (n=$n).")
...
println("=== Experiment 11: n_obs sweep, Lax–Wendroff, σ_proc = 0 ===")
...
p = plot(; xlabel="time (h)", ylabel="global RMSE(β)",
         title="Exp 11 — n_obs sweep (Lax–Wendroff, σ_proc = 0)",
